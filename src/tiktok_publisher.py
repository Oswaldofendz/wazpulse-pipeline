...
PHOTO field naming (different from VIDEO):
  - post_info.description = full caption text (visible on post)
  - post_info.title       = short label <=90 chars
  - source_info.photo_cover_index = 0-based index of cover slide
...
def _refresh_access_token() -> Optional[str]:
    ...
    Logs new tokens prominently -- user must update Railway env vars
    ...
    log.info("tiktok-pub: access_token refreshed (expires_in=%ss)", expires_in)

    if new_refresh and new_refresh != refresh_token:
        log.warning(
            "!! TIKTOK REFRESH_TOKEN ROTATED -- update Railway env var:\n"
...
def _query_creator_info(access_token: str) -> Optional[dict]:
    ...
    log.info("tiktok-pub: creator_info status=%s body=%s", r.status_code, body)
...
def _build_caption(post: dict) -> str:
    ...
    For PHOTO posts caption goes in the 'description' field.
    """
    flags  = post.get("compliance_flags") or {}
...
def _post_carousel(
    ...
    PHOTO posts field naming:
      - post_info.description = full caption (what users see)
      - post_info.title       = short label <=90 chars
      - source_info.photo_cover_index = 0-based cover slide index
    Images MUST be JPEG or WEBP (PNG is rejected with invalid_params).
    """
    headers = {
        "Authorization": f"Bearer {access_token}",
    }
    ...
    payload = {
        "post_info": {
            "title":         short_title,   # short label for PHOTO posts
            "description":   description,   # full caption (visible text)
    ...
    log.info(
        "tiktok-pub: payload privacy=%s photo_count=%d desc_len=%d title=%.80r",
        privacy_level,
        len(photo_urls),
        len(description),
        short_title,
    ...
    if r.status_code != 200:
        log.error("tiktok-pub: publish HTTP %s body=%s", r.status_code, body)
        return None
...
def _poll_publish_status(access_token: str, publish_id: str) -> str:
    """
    Poll TikTok until status resolves.
    Returns final status string or 'TIMEOUT'.
    """
    headers = {
        "Authorization": f"Bearer {access_token}",
        "Content-Type":  "application/json; charset=UTF-8",
